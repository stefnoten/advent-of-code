...
private class Day20 : AdventOfCodeSpec({ challenge ->
    challenge.map { parse(it) }
        .eval(35, 5179) { it.enhance().enhance().image.lightPixelCount }
        .eval(3351, 16112) { input -> (1..50).fold(input) { acc, _ -> acc.enhance() }.image.lightPixelCount }
}) {
    ...
    companion object {
        ...
        fun List<Int>.digitsToInt(bitsPerDigit: Int = 1) = fold(0) { acc, i -> (acc shl bitsPerDigit) + i }
    }
}
