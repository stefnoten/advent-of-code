package com.steno.adventofcode.y2021.day18
...
private fun main() {
    assignment("2021/day18") { it.map(::parseTree) }
        .eval { trees -> trees.reduce(Tree::plus).magnitude }
        .evalList { trees ->
            trees.maxOf { first ->
...
private fun parseTree(value: String): Tree = parseTree(value.asSequence())
private fun parseTree(line: Sequence<Char>): Tree = line.inOrder {
    when (val first = next()) {
        '[' -> Node(
